...
def run_monitoring_app():
    ...
    if EMAIL_ALERTS_AVAILABLE:
        with st.expander("📧 Email Alert Configuration", expanded=False):
            ...
            # Remove the secrets check and use direct input
            email_enabled = st.checkbox("Enable Email Alerts", value=False)
            recipient_email = st.text_input(
                "Recipient Email", 
                value="mostafanad2004@gmail.com",  # Set your default email here
                help="Email address to receive high error alerts"
            )
            
            # Add a test button that doesn't depend on secrets
