...
def run_forecast_app(model, prophet_df, model_type="unknown"):
    ...
    # ========================================================================
    # MODE 2: REAL-TIME PREDICTIONS (UPDATED VISUALIZATION)
    # ========================================================================
        ...
        if st.session_state.get('real_time_predictions'):
            ...
            if 'error' not in latest:
                ...
                # NEW: Enhanced Visualization with Historical Context
                display_real_time_with_history(latest, prophet_df, model_type)
...
def display_real_time_with_history(prediction_result, prophet_df, model_type):
...
def display_mlflow_forecast_results(forecast_data, prophet_df, model_type, end_date=None, periods=None):
    ...
    # 2. Chart - Enhanced to show historical + forecast clearly
    st.subheader("Forecast Visualization")
    fig = create_combined_chart(prophet_df, standardized_data, model_type)
...
def create_combined_chart(historical_df, forecast_df, model_type):
    """Create a chart that combines historical data and forecast clearly"""
    fig = go.Figure()

    # HISTORICAL DATA
    if not historical_df.empty:
        # Show last 180 days for context
